...
class Competition_Menu(Screen):
    ...
    def on_pre_enter(self):
        ...
        self.ids.competitions_layout_scrollview.scroll_y = 1
    ...
    def on_button_release(self, instance):
        ...
        if mode_training:
            # app.root.current = "competition_training"
            # app.root.transition.direction = "left"
            change_screen_to("competition_training", transition_direction="left")
            # continue game with selected competition
            competition_training_screen = app.root.get_screen("competition_training")
            competition_training_screen.select_competition(instance.text)
            competition_training_screen.play()

        if mode_game:
            # app.root.current = "competition_game"
            # app.root.transition.direction = "left"
            change_screen_to("competition_game", transition_direction="left")
            # continue game with selected competition
            competition_game_screen = app.root.get_screen("competition_game")
            competition_game_screen.select_city(self.selected_city)
            competition_game_screen.select_competition(instance.text)
            competition_game_screen.play()

        # adapt for competition
        # elif mode_browse:
        #     # change screen
        #     app.root.current = "firetruck_browse"
        #     app.root.transition.direction = "left"
        #     # continue game with selected firetruck
        #     firetruck_browse_screen = app.root.get_screen("firetruck_browse")
        #     firetruck_browse_screen.select_firetruck(instance.text)
        #     firetruck_browse_screen.populate_list()

        # adapt for competition
        # elif mode_images:
        #     app.root.current = "firetruck_images"
        #     app.root.transition.direction = "left"
